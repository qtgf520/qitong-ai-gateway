...
## 🔄 v3.1.0（当前最新）
### 🆕 新增
- **🔄 自动故障转移（Auto Failover）** — 首页网关开关下方新增「自动切换模型」开关，开启后首选模型不可用时自动切换到其他可用模型
- **智能模型选择** — 自动测速，优先选择响应最快的模型提供服务，支持文字/图片/视频/工具调用模型
- **自动重试机制** — 请求失败自动重试最多5次（指数退避 1s→2s→3s→4s→5s），5次全失败返回标准 OpenAI 错误
- **多模态自动适配** — 请求中的图片/工具等参数自动路由到支持对应能力的模型

### ⚡ 优化
- **网关全面 OpenAI 兼容** — 响应格式标准化 `id`/`object`/`created`/`model` 全部补齐
- **通知栏动态流量** — 像手机流量一样累计显示，永不归零，🟢传输中/⚪空闲指示灯
- **补充缺失 API 端点** — 新增 `/v1/embeddings`、`/v1/audio/transcriptions`、`/v1/completions` 代理支持
- **标准错误格式** — 所有错误返回 OpenAI 标准 `{"error":{"message":"...","type":"...","code":"..."}}`
- **服务商联动模型隐藏** — 禁用服务商→模型页/聊天/网关全部同步隐藏删除

### 🔧 修复
- ❌ 流式 JSON 响应格式不正确（Content-Type 非首次设置无效）→ ✅ call.respondBytesWriter 内设置 Content-Type
- ❌ deepseek 等慢模型流式易断开 → ✅ 自动重试5次 + 故障转移
- ❌ 通知栏清零导致网关逻辑中断 → ✅ 累计模式永不归零
- ❌ 清空日志按钮伪清空（追加"(已清空)"文字）→ ✅ 真清空 `debugLogBuffer.clear()`
- ❌ 抓包按钮颜色不随状态变化 → ✅ 改为 StateFlow，Compose 响应式更新
