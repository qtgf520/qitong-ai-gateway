# 綦桐AI网关 完整更新日志

> 包名：com.qtwl.gateway
> 签名证书：qitong.jks (别名: qitong)
> 最后更新：2026-09-07

---

## 🔄 v3.18.21（正式发布）

### 🐛 修复
- **启动闪退 NPE** — 修复 ViewModel `init` 构造期间启动协程访问未初始化 `_quickLiveSessions` 导致的 NPE 闪退。快照刷新协程延迟1秒、自动测速恢复延迟1.5秒启动，避开构造期

### ⚡ 性能优化（解决高版本Android卡顿/掉帧）
- **跑马灯卡顿修复** — 实时会话跑马灯由每800ms全页重组改为3秒低频快照刷新（`quickLiveSessions` StateFlow），消除首页高频重组
- **滚动卡顿修复** — LazyColumn每个item内重复订阅 forcedPool 提升到外层一次，滚动更流畅
- **首页精简** — 测速区域重复显示合并（当前AI助手模型/正在测速合并为单一简洁状态行）

### ✨ 新增
- **pID服务商ID可自定义** — 服务商ID可选自定义显示（SharedPreferences映射，避免数据库迁移），首页/排行榜/强制池统一使用 `displayId`
- **自动测速状态持久化恢复** — 启动APP时若上次自动故障转移/测速开启，自动恢复自动测速

### ✅ 验证
- 编译验证：`./gradlew assembleDebug` → `BUILD SUCCESSFUL`
- 安装验证：`pm install -r` → `Success`（versionCode=200, versionName=3.18.21）
- 签名：debug APK 使用 qitong.jks 签名，与 release 一致
- 启动稳定性：APP启动后进程存活，无闪退

---

## 🔄 v3.18.20（正式发布）

### 🎨 UI 重构
- **底部导航精简为4个** — 保留 首页/服务商/模型/聊天，底部菜单更清新简洁
- **右上角三点菜单** — 统计/管理/关于/实例 收纳到右上角 ⋮ 菜单，操作更集中

### 🔧 新增
- **多实例管理** — 支持创建/删除/切换多个网关实例（default/office/home等），各实例独立配置
- **modelMap 别名映射** — 自定义模型ID映射（如 `gpt-4` → `deepseek-chat`），实例级隔离
- **远程日志上报** — 可配置远程日志服务器地址，便于集中收集网关日志

### ⚡ 性能优化
- **修复高版本Android卡顿** — 优化 while 循环导致的高频重组，改用 StateFlow/snapshotFlow 减少不必要重组

### ✅ 验证
- 编译验证：`./gradlew assembleDebug` → `BUILD SUCCESSFUL`
- 安装验证：`pm install -r` → `Success`（versionCode=199, versionName=3.18.20）
- 签名：debug APK 使用 qitong.jks 签名，与 release 一致

---

## 🔄 v3.18.19-1（正式发布）

### 🐛 修复
- **🔁 网关HTTP流式故障转移真正触发** — 修复根因：`pipeStreamResponse` 流式路径读流异常被吞掉当EOF、写流异常只记日志，异常从不向上传播，导致外部APP走网关HTTP请求时，qtai-sj 首个模型一挂故障转移循环收不到异常、永不切换。现加**首块预读**——若上游连接建立后立刻失败/空流，在响应开始前安全抛异常，触发按强制池顺序切换到下一个模型
- **🔆 首页点灯切换生效** — 修复根因：点灯调用 `forceModel` 只保存强制模型key，未把点选的模型提升到强制池第一位（网关按池顺序优先用第一个），也未更新 `activeNodeName/providerId`，导致点灯不切换、灯不亮。现点灯时把该模型提升到池第一位并同步活跃节点，灯立即点亮，网关立即优先使用该模型

### ✅ 验证
- 编译验证：`./gradlew assembleDebug` → `BUILD SUCCESSFUL`
- 安装验证：`pm install -r` → `Success`（versionCode=196, versionName=3.18.19-1）
- 签名：debug APK 使用 qitong.jks 签名，与 release 一致

---

## 🔄 v3.18.19（正式发布）

### ✨ 新增
- **🔆 池灯显示 + 点灯手动切换** — 强制故障池内每个模型旁新增指示灯，灯亮=当前正在使用该模型（绿色可点）。点击灯可**手动切换**到该模型；自动故障切换后灯会自动点亮新模型。2秒轮询 `activeNodeName`+`activeNodeProviderId` 实时同步当前使用模型

### 🔧 优化
- **qtai-sj 强制故障转移真正生效** — 修复根因：qtai-sj 走的是独立"透传分支"，此前只选单个模型直接转发就 `return`，完全绕过强制池故障转移循环。现重构为**按强制池顺序逐个尝试**，出错自动切池内下一个模型，与网关其他路径行为一致
- **顺序=切换优先级** — ▲▼ 调整池内顺序后，故障切换真正按池顺序执行（1号→2号→3号...）
- **通知栏显示服务商ID** — 通知栏标题显示 `P{服务商ID} · 模型名`（如 `P2 · deepseek-v4-flash`），一眼看出当前使用的是哪个服务商的哪个模型
- **内部聊天选 qtai-sj 接入强制池** — 内置聊天选 qtai-sj 时，从"单服务商直连"重构为按强制池顺序构建候选列表，出错自动滚动切换下一个池内模型，与网关故障转移行为完全一致

### ✅ 验证
- 编译验证：`./gradlew assembleDebug` → `BUILD SUCCESSFUL`
- 安装验证：`pm install -r` → `Success`（versionCode=195, versionName=3.18.19）
- 签名：debug APK 使用 qitong.jks 签名，与 release 一致

---

## 🔄 v3.18.18-1（正式发布）

### 🐛 修复
- **🔥 强制故障池对所有请求生效，故障切换真正生效** — 修复根因：强制池此前只在 `qtai-sj` 请求时生效，当被调用APP请求**具体模型ID**时走 `autoFailover`（用户权威模式）分支，完全忽略强制池导致故障不切换。现重构 `attemptModels` 选择逻辑，强制池对所有请求优先生效（请求模型置首，其余按池内顺序作故障切换候选），故障时真正按池顺序自动切换

### ✅ 验证
- 编译验证：`./gradlew assembleDebug` → `BUILD SUCCESSFUL`
- 安装验证：`pm install -r` → `Success`（versionCode=194, versionName=3.18.18-1）
- 签名：debug APK 使用 qitong.jks 签名，与 release 一致

---

## 🔄 v3.18.18（正式发布）

### ✨ 新增
- **↕️ 强制故障池顺序调整** — 首页"强制故障多转移绑定"卡片内每个模型旁新增 ▲▼ 按钮，可调整切换顺序，**序号即故障切换优先级**（1号模型崩溃 → 自动切2号 → 3号...）
- **⚡ 模型自由调度（消除卡顿）** — 池内模型后台自动预热探活（健康缓存过期5分钟自动重新测速），配合强制池路由，故障时毫秒级切换到下一个可用模型，**被调用APP无感知**
- **🔥 立即预热按钮** — 卡片内新增"立即预热池内模型"按钮，可手动触发池内所有模型探活

### 🔧 优化
- **首页删除"首页思考引导"卡片** — 启动服务按钮下方、自动故障转移上方的思考引导提示已移除
- 强制池模型列表改为带序号和▲▼按钮的可排序布局

### 🐛 修复
- **上游空content触发故障转移** — 上游返回200但空body/空choices/空content时，抛出Upstream异常触发故障转移，不再返回500错误

### ✅ 验证
- 编译验证：`./gradlew assembleDebug` → `BUILD SUCCESSFUL`
- 安装验证：`pm install -r` → `Success`（versionCode=193, versionName=3.18.18）
- 签名：debug APK 使用 qitong.jks 签名，与 release 一致

---

## 🔄 v3.18.17（正式发布）

### ✨ 新增
- **🎯 强制故障多转移绑定** — 首页群聊模式下方新增"强制故障多转移绑定"管理卡片，强制选择模型可**多选无限加**组成故障池；默认模型崩溃后，核心路由自动按池内顺序切换到下一个可用模型，不再因单选而中断故障转移
- **📊 排行榜显示优化（测速/显示/删除三者解耦）** — 排行榜无需测速也显示所有已启用模型，新增模型直接上排行；测速只影响排序（已测速的排前、未测速的排后），不决定是否显示
- **🗂️ 强制故障池持久化** — `GatewayForegroundService` 新增 `getForcedPool`/`saveForcedPool`/`addToForcedPool`/`removeFromForcedPool` 方法，池内模型有序存储，单选中自动设为池内第一个

### 🔧 优化
- 排行榜点击行为：从"单选强制切换"改为"**加入/移出强制故障池**"
- 首页排行榜标题更新为"📊 模型排行榜（点击加入/移出强制故障池）"，强制模式指示改为显示"🎯 强制故障池 (N)"

### ✅ 验证
- 编译验证：`./gradlew assembleDebug` → `BUILD SUCCESSFUL`
- 安装验证：`pm install -r` → `Success`（versionCode=190, versionName=3.18.17）
- 签名：debug APK 使用 qitong.jks 签名，与 release 一致

---

## 🔄 v3.18.16（正式发布）

### 🐛 修复
- **💥 前台服务启动崩溃（ClassCastException）** — `GatewayForegroundService.getSavedTraffic()` 加 `try-catch ClassCastException` 兜底，兼容旧版 SharedPreferences 存成 String 类型导致闪退的问题
- **🔌 Ktor CIO 端口占用异常处理** — 启动前 `isPortAvailable()` 提前检测端口可用性，端口占用时自动尝试备选端口（+1起，最多20个），自动持久化备选端口避免下次再冲突，`stop()` 加 try-catch 确保端口完整释放

### ✨ 新增
- **🧠 模型独立记忆开关** — 数据管理页大脑记忆卡片新增"模型独立记忆"开关，开启后各模型记忆互相隔离，`BrainMemoryManager.addMemory()` 自动标注当前模型ID，`getSubconscious()` 只取当前模型相关记忆

### ✅ 验证
- 编译验证：`./gradlew assembleDebug` → `BUILD SUCCESSFUL`
- 安装验证：`pm install -r` → `Success`（versionCode=188, versionName=3.18.16）
- 签名：debug APK 使用 qitong.jks 签名，与 release 一致

---

## 🔄 v3.18.15-1（测试发布）

### 🐛 修复
- **💥 前台服务启动崩溃（ClassCastException）** — `GatewayForegroundService.onCreate()` 中 `getSavedTraffic()` 调用 `sp.getLong()` 时，因旧版数据可能存成了 String 类型，导致 `ClassCastException: String cannot be cast to Long`，APP 启动即闪退。修复方案：`getSavedTraffic()` 加 `try-catch ClassCastException` 兜底，捕获异常后回退到 `sp.getString()` + `toLongOrNull()` 兼容解析

### ✅ 验证
- 编译验证：`./gradlew assembleDebug` → `BUILD SUCCESSFUL`
- 安装验证：`pm install -r` → `Success`（versionCode=186, versionName=3.18.15-1）
- 签名：debug APK 使用 qitong.jks 签名，与 release 一致

---

## 🔄 v3.18.15（正式发布）

### ✨ 新增
- **🏷️ 模型选择器P前缀显示（全端统一）** — 群聊参与模型/总结模型选择、qtai-sj脑子按钮/弹窗、统计页模型选择器/趋势图标题、服务商页名称，全部统一显示 `P${providerId}·${modelId}` 格式
- **🔗 模型选择/去重改用 routeKey** — 群聊参与模型、总结者选择、GroupChatManager 内部匹配全部改用 `routeKey`（`providerId::modelId`）做唯一标识，彻底修复同名模型（如多个服务商均有 deepseek-v4）点击一个全选/错误路由的bug
- **⏱ 测速间隔滑块绑定自启测速** — 修改滑块间隔时，若测速正在运行，自动 `stopPipelineTest()` + `startPipelineTest()` 使新间隔立即生效，不再需要手动开关
- **📊 统计页"所有模型"选项** — 模型选择器新增 `📊 所有模型` 选项，点击清空选择显示全部模型历史趋势

### ✅ 验证
- 编译验证：`./gradlew assembleDebug` → `BUILD SUCCESSFUL`
- 安装验证：`pm install -r` → `Success`（versionCode=185, versionName=3.18.15）
- 签名：debug APK 使用 qitong.jks 签名，与 release 一致

---

## 🔄 v3.18.14（正式发布）

### ✨ 新增
- **🔇 批量测速错误自动关闭开关** — 模型页新增"🔇 错误自动关闭"开关，开启后批量测速中失败的模型会被自动禁用（`isEnabled=false`），避免反复调用无效模型，与"批量测速自动开启"按钮并列使用

### ✅ 验证
- 编译验证：`./gradlew assembleDebug` → `BUILD SUCCESSFUL`
- 安装验证：`pm install -r` → `Success`（versionCode=183, versionName=3.18.14）
- 签名：debug APK 使用 qitong.jks 签名，与 release 一致

---

## 🔄 v3.18.13（正式发布）

### 🐛 修复
- **Android 16前台服务闪退** — `ForegroundServiceDidNotStartInTimeException` 修复：`onCreate()` 最开头立即 `startForeground()` 创建默认通知，避免5秒超时崩溃
- **大脑模型选择跨服务商重名全选** — 大脑存储和比较从 `modelId` 改为 `routeKey`（`providerId::modelId`），跟测速排行一致，同名模型（如多个DeepSeek）不再全选标记

### ✨ 新增
- **备份覆盖更多设置项** — 导出白名单新增15个键（`pipeline_interval_minutes`、`show_home_hint`、`home_hint`、`home_thinking_depth`、`qtai_sj_brain_model`、`traffic_upload_total`、`traffic_download_total`、`proxy_list_json`、`allowed_api_keys`、`proxy_protocol`、`proxy_host`、`proxy_port`、`last_auto_backup_time`、`auto_backup_retention_days`、`initial_setup_done`），恢复逻辑补充布尔/整数类型分支
- **大脑模型选择UI按服务商分组** — DataManagementScreen大脑记忆卡片新增模型选择器，按排行榜风格（服务商分组，`P$providerId · $providerName` 标签）

### 🔧 优化
- **DEV_GUIDE.md 升级 v19** — 新增铁律15~18（签名环境变量/版本号递增/routeKey去重/项目级备份），新增第10节「AI经典踩坑清单」，第7节测试版流程增加①备份项目

### ✨ 新增（v3.18.12 补充）
- **🚀 网关启动自动测速** — `GatewayService.start()` 启动2秒后自动调用 `GatewayScheduler.refreshHealthCache(database)`，自动故障转移开启时触发，无需手动点测速
- **⏰ 网关启动自动调度备份** — 启动后检查 `auto_backup_enabled` 配置，启用时自动调用 `AutoBackupWorker.schedule()` 调度WorkManager定时备份
- **🏠 思考引导和群聊模式移到首页** — 从数据管理页移至首页，首页直接使用思考引导和群聊，操作更便捷
- **🔽 重置按钮移到底部** — DataManagementScreen 重置数据Card移至Column底部，添加服务Card之前，更符合操作流程
- **🔗 导出备份合并到备份&恢复卡片** — 删除独立导出行，整合到备份&恢复卡片内部，界面更简洁

### 🐛 修复（v3.18.12 补充）
- **🧹 qtai-sj无前缀输出人格移除** — 无前缀透传路径不再输出人格设定，纯净模型回复
- **💬 关于我们添加QQ群号** — 关于页面新增"QQ群: 966345026"，方便用户交流

### ✅ 验证
- 编译验证：`./gradlew assembleDebug` → `BUILD SUCCESSFUL`
- 安装验证：`pm install -r` → `Success`（versionCode=182, versionName=3.18.13）
- 签名：debug APK 使用 qitong.jks 签名，与 release 一致

---

## 🔄 v3.18.11（正式发布）

### 🐛 修复
- **服务商页UI添加chatPath** — `AddProviderDialog`和`EditProviderDialog`新增chatPath下拉选择框（ExposedDropdownMenuBox），支持自定义API路径选择（/v1/chat/completions、/v1/messages、/v1/completions等12种预设路径），空值自动转null
- **测速串行延迟** — `refreshHealthCache`从并发`async/awaitAll`改为串行`for`循环，每模型间加500ms延迟，避免并发测速被误判为异常
- **内置聊天不走chatPath** — `sendMessage`调用`UpstreamClient.requestStream`时传入`provider.chatPath`，内置聊天现在也使用自定义API路径
- **chatPath空值保存** — `saveProvider`中`chatPath = form.chatPath`改为`form.chatPath.ifBlank { null }`，空字符串存为null，数据库不存无效值

### ✨ 新增
- **管理页chatPath标签优化** — DataManagementScreen中chatPath标签改为"对话接口路径（留空自动拼接）"，添加URL预览实时显示baseUrl+chatPath组合结果

### ✅ 验证
- 编译验证：`./gradlew assembleDebug` → `BUILD SUCCESSFUL`
- 安装验证：`pm install -r` → `Success`（versionCode=177, versionName=3.18.11）
- 签名：debug APK 使用 qitong.jks 签名，与 release 一致

---

## 🔄 v3.18.9（正式发布）

### ✨ 新增
- **✏️ 手动添加模型** — 模型页新增"手动添加模型"按钮，选择服务商后直接输入模型ID（如 gpt-4o）即可使用，无需等待自动同步。自动同步+手动添加互补，解决部分服务商模型列表不全的问题
- **🔧 自定义API路径实时反映在最终URL** — Base URL输入框下方展示 `baseUrl + chatPath` 组合结果，告别硬编码

### 🔧 优化
- **API地址不再强制自动拼接 `/v1/chat/completions`** — 现在 Base URL 和 Chat API path 各自独立展示，最终URL实际反映 `baseUrl + chatPath` 组合，不再硬编码拼接
  - Ollama 用户：Base URL 填 `http://192.168.1.100:11434`，Chat API path 填 `/api/chat` 或 `/api/generate`
  - Gemini 用户：Base URL 填 `https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:generateContent`，Chat API path 留空
  - 中转/聚合服务：Base URL 填完整上游地址，Chat API path 留空或填自定义路径
- **提示文本更新** — 15种语言的 url_hint 从"自动拼接 /v1/chat/completions"改为"输入完整API地址，自定义路径在下方设置"

### 🐛 修复
- **💥 后端所有硬编码 `/v1/chat/completions` 路径全面修复** — 转发层（GatewayService / GatewayScheduler / ModelSpeedTester / ModelCapabilityManager / GatewayViewModel / GroupChatManager）全部改用 `provider.chatPath`，服务商自定义 `chatPath` 功能真正可用
  - `/v1/completions` 处理器：使用 `provider.chatPath` 拼接上游URL
  - `/v1/messages` 处理器：使用 `provider.chatPath` 拼接上游URL
  - 大脑（qtai-sj）模块转发：使用 `brainProvider.chatPath`
  - 健康检查路径：使用 `provider.chatPath` 探测
  - 测速引擎：`measure()` 新增 `chatPath` 参数，默认回退 `/v1/chat/completions`
  - 能力探测：`probeModel()` 新增 `chatPath` 参数，探测URL使用自定义路径
  - 批量测速：全部传入 `provider.chatPath` 替代硬编码

### ✅ 验证
- 编译验证：`./gradlew assembleDebug` → `BUILD SUCCESSFUL`
- 安装验证：`pm install -r` → `Success`（versionCode=175, versionName=3.18.9）
- 签名：debug APK 使用 qitong.jks 签名，与 release 一致

---

## 🔄 v3.18.8（正式发布）

### 🐛 修复
- **测速倒计时bug** — 倒计时循环中错误检查 `_pipelineRunning.value`（刚设为false），导致立即跳出，30秒就重启。修复后倒计时严格按滑块设定时间（5分钟=300秒）倒数，归零才重启测速
- 滑块时间绑定：5分钟 → 300秒倒计时，10分钟 → 600秒倒计时，以此类推

### ✅ 验证
- 编译验证：`./gradlew assembleDebug` → `BUILD SUCCESSFUL`
- 安装验证：`pm install -r` → `Success`（versionCode=173, versionName=3.18.8）
- 签名：debug APK 使用 qitong.jks 签名，与 release 一致

---

## 🔄 v3.18.7（正式发布）

### ✨ 新增
- **🔀 自定义路由规则引擎** — 支持路径/模型名(*通配符)/API密钥/服务商匹配，支持route转发+block拒绝
  - 规则管理UI：添加/编辑/删除/启用/清空，支持优先级排序
  - 路径匹配：`contains` 匹配请求路径
  - 模型名匹配：`*` 通配符，如 `gpt-*` 匹配所有GPT模型
  - API密钥前缀匹配：`startsWith` 匹配密钥前缀
  - 服务商匹配：精确匹配服务商ID
  - 动作：`route`（转发到指定模型）/ `block`（拒绝请求，返回403）
  - GatewayService集成：proxyRequest中自动匹配规则并执行动作
- **📊 模型延迟历史趋势图** — Canvas折线图展示TTFT/TPS/总耗时历史趋势，三指标切换，模型选择器
- **🔑 按API密钥用量面板** — 统计页新增按API密钥分组的用量统计，显示密钥名称/调用次数/输入输出Tokens/上传下载流量

### 🔧 优化
- 数据库版本升级 v9→v10：新增 `routing_rule` 表
- 新增 `RoutingRuleManager` 单例引擎，带30秒缓存
- GatewayViewModel 新增路由规则CRUD方法
- DataManagementScreen 新增路由规则管理卡片

### ✅ 验证
- 编译验证：`./gradlew assembleDebug` → `BUILD SUCCESSFUL`
- 安装验证：`pm install -r` → `Success`（versionCode=172, versionName=3.18.7）
- 签名：debug APK 使用 qitong.jks 签名，与 release 一致

---

## 🔄 v3.18.7-1（测试发布）

### ✨ 新增
- **📊 模型延迟历史趋势图** — 每次测速自动记录TTFT/TPS/总耗时到数据库，统计页Canvas折线图展示历史趋势
  - 三指标切换：FilterChip 切换 TTFT / TPS / 总耗时
  - 模型选择器：点击"所有模型"按钮可切换查看单个模型的历史趋势，或显示所有模型最新记录
  - 统计摘要：最新值 / 平均值 / 数据点数
  - 自动记录：流水线测速和单模型测速均自动写入数据库，保留7天数据
  - 自动清理：每次写入时自动清理7天前的旧记录
- **数据库版本升级 v7→v8** — 新增 `speed_history` 表（id, model_key, model_name, provider_id, ttft_ms, tps, total_ms, success, measured_at）

### ✅ 验证
- 编译验证：`./gradlew assembleDebug` → `BUILD SUCCESSFUL`
- 安装验证：`pm install -r` → `Success`（versionCode=169, versionName=3.18.7-1）
- 签名：debug APK 使用 qitong.jks 签名，与 release 一致

---

## 🔄 v3.18.6（正式发布）

### ✨ 新增
- **⏱ 定时测速倒计时显示** — 测速跑完一轮后，首页滑块下方显示 `⏱ 下一轮测速: MM:SS` 倒计时，归零自动重启测速
- **🏆 排行榜三行布局** — 每个测速完成项独立三行显示：
  - 第一行：`#排名 · P服务商ID · 模型名`（带选中标记🎯）
  - 第二行：`服务商名 · modelId`
  - 第三行：测速指标（`✅ TTFT=123ms TPS=45 总耗时=456ms` 或 `❌ 不可用`）

### 🔧 优化
- **手动点击测速重置定时器** — 手动点击测速按钮时，自动重置倒计时，避免等待上一轮剩余时间
- **测速开关绑定更直观** — 开关关闭时倒计时清除，开启时从零开始

### ✅ 验证
- 编译验证：`./gradlew assembleDebug` → `BUILD SUCCESSFUL`
- 安装验证：`pm install -r` → `Success`
- 签名：debug APK 使用 qitong.jks 签名，与 release 一致

---

## 🔄 v3.18.5（正式发布）

> 开发：adybag14-cyber（GitHub PR #3）

### 🐛 修复
- **服务商作用域模型排序选择修复** — 模型排行榜按服务商作用域隔离，避免跨服务商数据混淆，不同服务商下的同名模型作为独立模型处理。
- **同名模型不再串选** — 排行、测速缓存、健康状态、自动故障转移和强制选择统一使用 `providerId::modelId`，不同服务商下的 `Haven` 会作为两个独立模型处理。
- **旧缓存安全迁移** — 旧版只保存 `modelId` 的记录仅在全局唯一时迁移；遇到重名模型不再默认取第一个服务商。
- **首页排行可明确选择** — 每行显示 `#排行ID · P服务商ID · 模型名`，点击排行行时按该行的服务商和模型精确路由。
- **数字指令精确切换** — 按排行编号切换时保存完整服务商路由键，发送到上游的 `model` 字段仍保持真实模型 ID。
- **模型分组不再合并同名服务商** — 模型设置页按服务商数据库 ID 分组，并显示 `P服务商ID · 服务商名`。
- **恢复 Android 备份规则资源** — 恢复 Manifest 引用但上游最新提交中缺失的 `backup_rules.xml`，保证 APK 可正常构建。

### ✅ 验证
- Windows: `testDebugUnitTest`, `assembleDebug`, `lintDebug`
- 13 项 JVM 单元测试通过，其中 4 项覆盖重复模型名、旧缓存歧义和排行顺序。
- 新增 `ModelRouteKey.kt` 用于模型路由键管理。

---

## 🔄 v3.18.4（正式发布）

### ✨ 新增
- **⏱ 自动测速间隔可配置** — 首页测速排行榜新增滑块，5分钟~4小时自由调节，跑完一圈自动停，按设定时间再跑
- **🔔 通知栏正文精简** — 去掉展开通知中代理和流量中间的模型名行，只保留标题中的模型名

### 🐛 修复
- **通知栏正文不显示冗余模型名** — 展开通知不再出现重复的模型名行，更清爽

### 🔧 优化
- **测速不再一直跑** — 测速跑完一圈后自动停止，等待设定间隔时间后再重新跑一轮，手动点击测速也会重置定时器

---

## 🔄 v3.18.3（正式发布）

### 🐛 修复
- **通知栏模型名闪烁彻底修复** — `startForeground()` 改为首次初始化用，后续用 `NotificationManager.notify()` 轻量更新，不再重建通知
- **通知栏流量刷新不闪** — 内容无变化时直接跳过，不触发任何通知操作

### 🛠️ 优化
- **DEV_GUIDE.md 全面升级 v16** — 新增铁律0（开发前必读指南）、铁律8（工具编辑替代Vim）、版本号示例更新到3.18.x、新增备份目录规则、验证清单补充通知栏不闪烁+备份恢复验证、Release上传名带版本号
- **DEV_GUIDE.md 升级 v17** — 新增铁律12：每次发布前同步更新README.md和CHANGELOG.md；正式发布流程③更新README.md ④更新CHANGELOG.md
- **README.md 更新** — 版本号更新到v3.18.3，版本历史表新增v3.18.3和v3.18.2
- **备份目录迁移** — 从 `/sdcard/Download/QiTongGateway/` 迁移到 `/sdcard/Download/Operit/backup/qtkfbf/`，`getBackupDir()` 统一返回新路径
- **备份保留策略优化** — 从保留最近7份改为保留最近5天，`cleanupOldBackups(5)` 按时间戳清理超5天的旧备份

---

## 🔄 v3.18.2（正式发布）

### 🐛 修复
- **管理页重复了两个「隐藏多任务」** — 删除了Git冲突合并导致的重复卡片，仅保留自启管理卡片内的开关
- **同名模型在不同服务商下无法区分** — 聊天选模型时显示 `modelId · 服务商名`，一眼区分
- **Git冲突残留导致编译失败** — 清理了 `build.gradle.kts`、`GatewayService.kt`、`MainScreen.kt`、`MainActivity.kt`、`GatewayViewModel.kt` 共5个文件的冲突标记
- **通知栏闪烁** — `updateNotification()` 内容无变化时跳过重建
- **ViewModel编译错误** — `liveSessions` 的 `set(value)` 语法修正

### ✨ 新增
- **聊天选模型显示服务商名** — 同名模型一眼区分，不会选错

---

## 🔄 v3.18.1（测试版）

### 🐛 修复
- **技能回复暴露指令编码** — 查排行/查状态等回复不再显示【指令:600002】，全部自然语言输出
- **qtai-sj 选模型 Token 统计缺失** — 透传路径缺少 `modelId/providerId` 属性，统计页查不到模型用量
- **模型页按钮挤名字** — 操作按钮改到第二行，第一行只显示模型名+ID
- **技能管理页背景透明** — 全屏覆盖加 surface 背景色

### ✨ 新增
- **👻 隐藏多任务开关** — 管理页自启管理下新增开关，开启后APP不在最近任务显示
- **🔼 服务商长按排序** — 长按弹出上移/下移菜单，顺序写入数据库持久化
- **🗑️ 模型可删除** — 不需要/不可用的模型可直接删除
- **💾 全部设置可备份/恢复** — 备份数据升级到 v4，包含所有 SharedPreferences 配置
- **🏃 不等测速即可用** — 查排行/聊天/群聊选模型不再等测速完成，缓存数据直接可用
- **⏰ 测速开关持久化** — 关闭测速后重启APP不再自动启动

### 🔧 优化
- **模型按服务商排序** — 服务商拖动排序后，模型列表自动跟随
- **聊天选模型按排行排序** — 跑得快的排前面，未测速的排最后
- **群聊选模型按排行排序** — 同上
- **qtai-sj 脑子选择加回 RadioButton** — 当前选中的模型一目了然

---

## 🔄 v3.18.1（正式发布）

### 🙇 致歉
> 非常抱歉！上一个更新版本存在网关崩溃和500错误，影响了部分用户正常使用。本次已彻底修复，并恢复了模型双排布局等全部功能。感谢大家的耐心与包容！

### 🐛 修复
- **网关500错误** — 移除了破坏请求体的tools过滤正则，上游不再报"field Messages invalid"
- **通知栏模型名频繁闪烁** — 4处 `activeNodeName` 赋值均改为仅变化时更新，不再每秒刷新
- **sanitizeRequestBody 函数体被截断** — 已从备份恢复完整参数修正逻辑

### ✨ 新增（v3.18.1 功能补全）
- **模型页双排布局** — ModelCard 分两行显示（名称+ID / 操作按钮），按钮带文字标签
- **服务商按 orderIndex 排序** — 长按可上移/下移调整顺序
- **模型选择器按测速排行排序** — 跑得快的排前面，未测速的排最后
- **删除模型按钮** — 模型卡片支持直接删除模型
- **隐藏多任务开关** — 管理页新增「👻 隐藏多任务」选项
- **备份设置恢复** — 备份版本升级到v4，支持导出/导入全部设置配置
- **测速缓存实时更新** — PipelineSortedModelIds 在测速过程中同步更新，不等测完就能用
- **测速开关持久化** — 关闭测速后重启APP不再自动启动测速
- **查排行无数据友好提示** — 显示缓存数据或引导文案，不再直接报"暂无数据"

---

### 🐛 修复
- **首页测速列表遮挡模型名** — 改为双行布局，模型名独占一行，指标/状态第二行
- **定时备份开关开启后不生效** — 放宽约束(取消网络/电量要求)、KEEP策略防重复、启动时从持久化恢复调度
- **qtai-sj 测速状态异常** — 兼容非SSE完整JSON响应、30秒总超时兜底、TTFT=-1标记失败
- **全部测速完后首页不显示 qtai-sj 可用** — 新增独立 QtaiSjStatus 状态管理 + 状态卡片

### ✨ 新增
- **P1 双行排行卡片** — 排名+模型名独占一行，指标/状态清晰分离
- **P2 测试备份按钮** — 设置页新增「🧪 测试备份（10秒后执行）」按钮
- **P4 qtai-sj 状态卡片** — 首页显示「⏳测速中/✅可用→路由到xxx/❌暂无可用模型」
- **AutoBackupWorker 防重复调度** — 检查已调度状态，避免 REPLACE 重置24小时计时器

### 🔧 优化
- `ModelSpeedTester` 支持非SSE响应解析（兼容非流式API）
- 定时备份约束放宽：不要求网络、不要求电量
- `ExistingPeriodicWorkPolicy.KEEP` 保持已有调度，不重置
- 启动时从持久化恢复定时备份调度

---

## 🔄 v3.17.2（正式发布）

### ✨ 新增
- **📊 统计页「清空流量」按钮** — 点击清除总上传+总下载+持久化流量统计，与「清除数据」（Token用量）并列
- **🧹 `clearTotalTraffic()` 方法** — GatewayForegroundService 新增，同时清除通知栏+总统计+持久化

### 🐛 修复
- **统计页当前模型不刷新** — `activeNodeName` 改为 `LaunchedEffect` 轮询方式（2秒），Compose 实时感知变化
- **版本号统一** — 本地/远程/Release 全部统一为 3.17.2

---

## 🔄 v3.17.1（正式发布）

### 🆕 新增
- **🎯 三层技能路由器** — `SkillRouter.kt`，关键词匹配(Layer 1)→语义匹配(Layer 2)→LLM判断(Layer 3)，自然语言通杀全部技能
- **🔍 搜索技能** — `SearchSkill.kt`，SearXNG集成，智能判断是否需要联网搜索
- **🧑 人格系统升级** — 大五人格维度滑块（开放性/尽责性/外向性/宜人性/神经质/幽默感/共情力），管理页可调
- **💾 备份完善** — WorkManager定时自动备份（`AutoBackupWorker.kt`），应用启动时恢复调度
- **⚠️ 重置确认升级** — 需输入"确认重置"才能清空数据
- **📲 转发处理** — `ReceiveShareActivity.kt`，接收外部ACTION_SEND转发消息
- **🔄 应用启动恢复定时备份** — GatewayApplication.onCreate() 自动恢复WorkManager调度

### 🐛 修复
- **同名模型全选bug** — 排行榜强制选择用 `providerId::modelId` 精确匹配，不再一次选两个同名模型
- **模型分类恢复** — 改回按服务商分组，不是按 modelId 分组
- **统计页不显示模型** — 改用轮询方式同步 `activeNodeName`

### 🎯 三指标测速 + 能力标签 + 搜索 + 备份升级

#### 🆕 新增
- **⚡ 三指标测速** — TTFT（首字延迟）/ TPS（每秒token）/ 总耗时，OkHttp SSE流式精准采集
- **🏷️ 9维能力标签** — 🔧工具调用 👁️视觉 🧠深度思考 🎤语音输入 🔊语音输出 🎬视频 🎨图像生成 📐向量嵌入 ⚡实时对话
- **🔍 同名模型分组** — 按 modelId 分组，展开显示供应商子项，解决同名模型混淆
- **🔖 标签筛选** — FilterChip 过滤，快速找到支持特定能力的模型
- **🔍 SearXNG 搜索** — `/v1/search?q=xxx` 接口，自建搜索引擎零成本无限制
- **💾 备份升级** — GZIP压缩 + SHA256校验 + AES-256-GCM加密，.qtbk格式

#### 🐛 修复
- **ModelsScreen 花括号错乱** — 重构搜索+筛选+分组逻辑，代码结构清晰
- **备份文件过大** — GZIP压缩减少70%体积
- **备份无校验** — SHA256完整性校验，损坏文件拒绝导入

---

## 🔄 v3.14.0（正式发布）

### 🐛 修复

#### 🖼️ 图像生成接口修复
- **404 Not Found** — 请求体中 `model: "qtai-sj"` 未替换为真实模型ID，导致找不到模型
- **新增 `replaceModelInBody()`** — 统一替换请求体中的 model 字段
- **兜底机制** — 找不到匹配模型时用第一个已启用的模型

---

## 🔄 v3.13.0（正式发布）

### 🎯 綦小桐技能管理系统 + 连续对话模式

#### 🆕 新增
- **💬 连续对话模式** — 开启后所有消息自动走綦小桐大脑，无需喊前缀
- **📋 技能管理页面** — 独立全屏页面，内置技能只读展示，自定义技能完全管理
- **🧩 自定义技能** — 手动添加/编辑/删除，支持名称/描述/关键词/提示词
- **📥 Git导入技能** — 从 raw.githubusercontent.com 的 raw 链接导入技能 JSON 数组
- **🤖 綦小桐技能管理卡片** — 数据管理页新增，连续对话开关 + 管理技能按钮

#### 🐛 修复
- **技能管理页面Scaffold嵌套崩溃** — 移出到Box + Column，避免双Scaffold冲突
- **点击管理技能闪退** — `showSkillManagement` 移到 Scaffold 外部，与密钥管理页一致

---

## 🔄 v3.12.0（正式发布）

### 🛡️ 全面升级：崩溃捕获 + 自动更新 + 网关智能检测 + 搜索技能

#### 🆕 新增
- **💥 崩溃日志捕获** — 全局未捕获异常自动保存完整堆栈+设备信息到本地
- **📤 崩溃提交反馈** — 关于页面检测到崩溃日志时显示红色卡片，支持复制日志/提交GitHub Issues/清除
- **🔄 自动更新检查** — 关于页面新增检查更新卡片，自动查询GitHub最新Release并比较版本号
- **🔍 多搜平台集成** — 綦小桐技能池新增搜索技能（000001搜索引擎/000002搜图），调用DuckDuckGo API
- **🧠 网关状态智能检测** — ViewModel初始化时通过`ActivityManager.getRunningServices()`双重确认，进程被杀死后APP不再显示"停止网关"

#### 🐛 修复
- **后台进程被杀死后状态错误** — 显示"停止网关"实际服务已死 → 智能检测修正为false

---

## 🔄 v3.11.1（正式发布）

### 🐛 修复：请求体校验全面增强

| # | 场景 | 修复前 | 修复后 |
|---|------|--------|--------|
| 1 | **缺失 `messages` 字段** | ⏳ 挂起超时 | ✅ 400 标准错误 |
| 2 | **缺失 `model` 字段** | ❌ 500 空body | ✅ 400 标准错误 |
| 3 | **空body `{}`** | ❌ 500 空body | ✅ 400 标准错误 |
| 4 | **`messages` 不是数组** | ❌ 500 空body | ✅ 400 标准错误 |
| 5 | **空 `messages` 数组** | ✅ 已修 | ✅ 继续保留 |

### ✨ 改进
- **全面请求体校验** — 所有错误情况返回标准400 OpenAI JSON错误，绝不挂起
- **CORS 全覆盖** — 所有错误响应也带 `Access-Control-Allow-Origin: *`
- **`reasoning_content` 兼容** — `ignoreUnknownKeys=true` 已支持非标准字段

---

## 🔄 v3.11.0（正式发布）

### 🎯 修复：外部测试 6 个兼容性问题

| # | 问题 | 修复 |
|---|------|------|
| 1 | **CORS 跨域缺失** | 新增 `OPTIONS` 预检路由，所有响应加 `Access-Control-Allow-Origin: *` |
| 2 | **GET /v1/chat/completions 返回500** | 改为返回 400 标准 OpenAI 错误 `{"error":{"message":"...","code":400}}` |
| 3 | **空 messages 数组导致挂起** | 新增空 messages 检测，返回 400 `"messages array is empty"` |
| 4 | **500 错误响应体为空** | 统一使用 `openAIError()` 返回 JSON 格式错误体 |
| 5 | **非标准 stream 格式** | `ignoreUnknownKeys=true` 兼容 `reasoning_content` 等额外字段 |
| 6 | **路径不统一** | 新增 `POST /chat/completions`（不带 v1 前缀）兼容路径 |

### ✨ 其他改进
- **CORS 全覆盖** — `corsResponse()` 在 `proxyRequest` 开头调用，所有路由自动加 CORS 头
- **空 body 保护** — `call.receive<ByteArray>()` 加 try-catch，空 body 不崩溃
- **qtai-sj 全接口适配** — 所有 16 个接口均支持 `model: "qtai-sj"` 自动解析为当前活跃模型
- **群聊不喊前缀** — 开启群聊后直接走群聊引擎，无需喊綦小桐

---

## 🔄 v3.10.0（正式发布）

### 🚀 重大更新：完整 API 接口适配

网关现已完整支持 OpenAI / Claude / Gemini 三大原生格式，总计 **16 个接口**：

| 接口 | 路径 | 说明 |
|------|------|------|
| 模型列表 | GET `/v1/models` | ✅ |
| 对话补全 | POST `/v1/chat/completions` | ✅ |
| 文本补全 | POST `/v1/completions` | ✅ |
| Claude消息 | POST `/v1/messages` | ✅ |
| 嵌入向量 | POST `/v1/embeddings` | ✅ |
| 重排序 | POST `/v1/rerank` | ✅ |
| 内容审核 | POST `/v1/moderations` | ✅ |
| 文本转语音 | POST `/v1/audio/speech` | ✅ |
| 图像生成 | POST `/v1/images/generations` | ✅ |
| 视频生成 | POST `/v1/videos` | ✅ |
| 视频任务 | POST `/v1/video/generations` | ✅ |
| 视频状态 | GET `/v1/video/generations/{task_id}` | ✅ |
| Gemini生成 | POST `/v1beta/models/{model}:generateContent` | ✅ |
| 引擎嵌入 | POST `/v1/engines/{model}/embeddings` | ✅ |
| 实时语音 | WS `/v1/realtime` | ✅ |

### 🔑 新增：API 密钥管理系统
- **管理页新增「密钥管理」入口** — 独立全屏页面
- **添加/删除/编辑密钥** — 每把钥匙可单独控制：
  - 启用/禁用
  - 备注标签
  - 限制可访问的模型列表
  - qtai-sj 独立访问权限开关
- **本地免密钥** — localhost/127.0.0.1/192.168.* 自动放行
- **密钥可备份** — 纳入备份/恢复系统，恢复时自动清空旧密钥导入新密钥

### ✨ 其他新功能
- **启动权限检查** — 打开APP自动检测通知权限、电池优化、唤醒保活
- **自启网关状态恢复** — 进程重建时恢复上次网关运行状态
- **群聊不喊前缀** — 开启群聊后直接走群聊引擎，无需喊綦小桐

### 🐛 修复
- **群聊开关自动回弹** — 不再因无参与者自动关闭
- **后台关屏掉线** — WakeLock + AlarmManager 双保活
- **自启网关状态恢复** — `Application.onCreate()` 阶段同步标记 `isServiceRunning`
- **首页换行符** — `\\n` → `\n`，快速上手说明正常显示
- **人格设定开关** — 实时读取配置，避免 stale 数据
- **WebSocket 闪退** — 修复 `WebSockets` 插件未安装导致的崩溃

### 📋 其他
- 所有接口均支持 `model: "qtai-sj"` 自动解析为当前活跃模型
- 所有接口均经过 API 密钥验证 + 流量统计 + 访问日志
- 多语言适配
- 添加服务商时支持自定义 API 路径（对话接口/模型列表接口）
- DEV_GUIDE.md 新增铁律11（发布不泄漏本地凭证）
- 清理 Git 历史中的敏感文件

## 🔄 v3.9.2（正式发布）

### 🐛 核心修复

#### 🧠 大脑流式修复
- **大脑 stream=true 返回空** — 修复大脑路径 stream=true 时 SSE 流解析失败导致返回空内容的问题
- **SSE 流解析** — 大脑路径新增 `data: {}` 逐行解析，正确提取 `delta.content`
- **usage 统计** — stream 模式下从 SSE chunk 中提取 usage 数据

#### 🔄 qtai-sj 透传优化
- **参数修正** — qtai-sj 透传路径增加 `sanitizeRequestBody()` 参数校验
- **model 替换** — 改用正则只替换第一个 model 字段，避免全局替换损坏 JSON
- **persona 注入** — 透传路径支持可选人格注入，与正常模型路径对齐

#### 📋 其他
- 备份文件清理（`.bak` 文件）

---

## 🔄 v3.9.1（正式发布）

> 开发：Adybag14-赛博（GitHub PR #2）

### 🌐 多语言修复

#### 🔧 修复（由 Adybag14-赛博 开发）
- **英文回退机制修复** — 切换到英文或其他非中文语言后，多个页面、通知、状态消息和内置AI提示仍显示中文的问题已修复
- **UI页面本地化** — 聊天、数据管理、首页、抓包、设置、统计等页面补充英文显示
- **通知与状态跟随语言** — 服务通知、群聊、思考引导、运行时状态和内置默认名称会跟随当前语言
- **非中文语言回退改进** — 缺少专用翻译时回退到英文，不再回退到简体中文
- **用户内容保护** — 保留用户自定义名称和内容，不会误翻译
- **minSdk 24 兼容** — 用 `Calendar` 替代 API 26 的 `LocalDateTime`，兼容 minSdk 24

#### 🧪 测试覆盖
- 新增单元测试：语言回退、双向状态翻译和自定义内容保护
- Android API 35 模拟器验证：英文→简体中文→English 切换通过
- 最终英文界面无可见中文文本节点

#### 📋 其他
- Lint 报告清理：移除 `LocalDateTime` API 26 警告
- 已知：通知/前台服务代码中仍有 5 个 `NewApi` 警告（非本次改动引入）

---

## 🔄 v3.9.0（正式发布）

### 🐛 核心修复

#### 通知栏流量统计
- **`resetNotificationTraffic()` 位置修复** — 从 proxyRequest 中的上传计数后调用改为最开头（所有计数之前），避免刚记的上传被清零
  - v3.9.0-1 修复：当前会话上传永远为 0 的问题
- **流式路径 `totalDownloadBytes` 缺失** — `pipeStreamResponse` 流式路径只更新通知栏下载，忘了同时更新总下载统计
  - v3.9.0-1 修复：总下载统计不增长

#### 模型使用统计
- **透传路径缺 `recordModelUsage()`** — qtai-sj无前缀透传路径转发后没有调用 `recordModelUsage(finalTarget.modelId)`，统计页"按模型统计(Top 10)"漏记
  - v3.9.0-2 修复：转发真实模型不再漏统计

#### 大脑直连
- **自建 OkHttpClient 不走代理** — 大脑(qtai-sj)路径自建了全新的 OkHttpClient（无代理配置），完全绕过 UpstreamClient 代理系统
  - v3.8.9-4 修复：改用 `UpstreamClient.getDirectClient()` 直连

#### 多语言
- **TranslationManager 新增 ~50 个翻译键** — 底部导航标题、首页网关状态/端口/局域网/服务商管理（添加/编辑/搜索）/模型别名编辑等文字全部可翻译
- **测试覆盖** — 已接入底部导航、顶部标题、首页卡片等位置
### 🧠 技能编码参数传递 + 前缀无空格匹配 + 兜底fallback修复

#### 🚀 新增
- **技能编码支持参数传递** — `【指令:200001:1】`格式，大脑可直接传模型编号/名称
- **200001/200002/100001支持参数执行** — 按编号切模型、按名称模糊匹配、单次测速
- **兜底fallback机制** — 大脑未输出编码时走旧式硬指令匹配，不遗漏任何指令

#### 🔧 修复
- **前缀无空格匹配修复** — `"綦小桐你多大了"`不带空格也能触发大脑
- **群聊空参与者自动关闭** — 防止阻塞大脑路径
- **技能执行结果直接拼入回复** — 大脑输出编码后系统执行并返回结果
- **WakeLock保活** — 添加`PARTIAL_WAKE_LOCK`防止后台掉线

#### 🧪 测试版迭代
- v3.8.9-1（code 115）— WakeLock保活 + 群聊修复 + 技能结果拼入
- v3.8.9-2（code 116）— 参数传递 + 兜底fallback
- v3.8.9-3（code 117）— 前缀无空格匹配修复

---

## 🔄 v3.8.8（正式发布）
### 🧠 人格系统注入大脑 + 当前模型同步

#### 🔧 修复
- **人格信息注入大脑Prompt** — 管理页设定的人格信息（名字/年龄/性格/背景）现在会同步给大脑，问"几岁了"大脑能答
- **当前活动模型同步大脑** — 大脑知道当前活动的真实模型名称，问"当前啥模型"能正确回答

#### 📋 其他
- 代码结构完整，花括号平衡 427/427
- 严格按DEV_GUIDE.md对照备份法开发

---

## 🔄 v3.8.7（正式发布 / 最终版）
### 🧠 綦小桐大脑调度系统 — 技能编码版
#### 🌟 核心架构重构
- **大脑调度取代关键词匹配** — 喊綦小桐后直接进大脑理解，大脑从技能池选最合适的技能执行，不再靠"关键词=动作"死匹配
- **52个技能全覆盖** — 测速/模型切换/网关控制/代理/数据/查询/导航/服务商管理/高级功能，整个APP全可控
- **技能编码系统** — 每个技能有唯一6位编码（如600002=查排行），对外完全不可见，大脑内部分配

#### 💡 智能调度逻辑
- 用户自然语言表达 → 大脑理解语义 → 大脑选择编码技能 → 系统执行 → 大脑组织回复
- 同一件事多种说法都能命中（"排行""哪个最快""排名"→600002）
- 新增功能只需往技能池加编码，不用改匹配规则

#### 🔧 最终修复
- **大脑检测后跳过硬匹配** — 有脑子时走if/else分支直接进大脑，不再执行旧式`parseCommand`
- **大脑Prompt注入技能表** — 大脑知道全部52个技能编码，回复时附带【指令:编码】
- **指令执行改用技能编码** — `extractSkillCodes`+`executeByCode`取代`parseCommand`+`execute`

#### 📊 通知栏流量统计升级
- **双轨制显示** — 通知栏同时显示"当前会话流量（可重置）"和"总统计（持久化）"
- **会话超时自动清零** — 所有会话过期时自动清空通知栏流量

#### 💾 全模型统计→数据库
- TokenUsage表新增 `upload_bytes`/`download_bytes` 字段
- 每次请求记录上传/下载字节到Room数据库
- 数据库迁移6→7自动加列，不丢数据

#### 📋 其他改进
- 通知栏和统计页同步显示总流量
- 旧式硬指令匹配保留（无脑子模型时兼容）
- 调试日志增强（技能执行记录）

---

## 🔄 v3.8.6（正式发布）
### 🐛 修复
- **通知栏切换模型不显示** — 透传路径增加 `activeNodeName` 同步和 `resetNotificationTraffic()` 调用，确保通知栏正确显示当前模型名
- **qtai-sj无前缀透传无回应** — 透明透传路径增加 `finalTarget` 三级兜底（排行榜→activeNodeName→任意已启用模型），全部模型不可用时返回503 JSON错误而非静默失败

### ✨ 新功能
- **会话延迟断开（自适应）** — 每个API会话按闲置超时自动清理，超时时间根据最近5次请求平均延迟自适应调节（基数30秒+延迟×2/1000，范围10-120秒），清理协程每10秒扫描过期会话

### 🛠️ 技术改进
- 新增 `sessionLastActive`/`sessionLatencyHistory` 双Map追踪会话活跃时间和历史延迟
- 新增 `getAdaptiveTimeout()` 自适应超时计算函数
- 新增 `cleanupExpiredSessions()` 定期清理协程
- 所有模型不可用时返回503 JSON错误而非静默丢弃

---

## 🔄 v3.8.5（正式发布）
### 🐛 修复
- **群聊选模型弹窗列表跳动** — 弹窗打开时使用 `remember` 快照数据（`viewModel.enabledModels.value` + `pipelineSortedModelIds.toList()`），避免测速排行榜实时刷新导致勾选位置不断跳动

---

## 🔄 v3.8.4（正式发布）
### ✨ 新功能
- **通知栏流量统计策略优化** — 软件重启或切换模型时，通知栏流量清零重新累计（按会话/按模型）；APP内存储统计保持不变（持久化不重置）
- **群聊模式改进** — 选模型方式从手动输入模型ID改为读取测速排行榜弹窗勾选添加，参与模型+总结模型均支持排行榜排序显示
- **清理旧APK文件** — `/sdcard/Download/` 中的38个旧版本APK已清理，仅保留最新版

### 🛠️ 技术改进
- 新增 `totalUploadBytes`/`totalDownloadBytes` 持久化总统计字段，与 `trafficUploadBytes`/`trafficDownloadBytes`（通知栏可重置）分离
- 新增 `resetNotificationTraffic()` 方法，切换模型时自动重置通知栏流量
- 流量统计双轨制：上传/下载均同时计入通知栏 + 总统计

## 🔄 v3.8.0（正式发布）
### 🚀 多模态+思考引导+群聊模式+全模型统计+短ID恢复
- 🆕 **多模态支持（图片→自动切视觉模型）** — 检测请求含 image_url 时，自动切换至支持视觉的模型
- 🆕 **思考引导可配置** — 管理页新增「🧠 思考引导」卡片，开关+深度选择（关闭/轻度/深度），脑子回复前自动注入思考 prompt
- 🆕 **群聊模式（虚拟沙箱+AI互聊+总结）** — 用户选择N个模型+1个总结者，发消息→AI依次发言→总结者输出结论，AI之间用 [@名称] 互相引用
- 🆕 **全模型流量统计统一入口** — `proxyRequest` 开头统一计 `trafficUploadBytes`，去重6处冗余计费
- 🆕 **流量统计持久化** — 每10秒自动保存到 SharedPreferences，重启不再清零
- 🆕 **统计页显示当前活跃模型** — 网关流量卡片新增 🧠 当前模型行
- 🔧 **恢复短ID匹配** — qtai-sj 透传路径支持完整前缀和短ID两种格式
- 🔧 **刀6修复** — 脑子逻辑加 `actualCmd.isNotBlank()` 保护，无前缀不走脑子
- 🔧 **刀4+刀8修复** — 脑子 catch 返回502错误，不再静默空转
- 🔧 **刀9修复** — brainBody 的 `stream` 参数透传，支持流式
### 🚀 qtai-sj无前缀转发修复 + 通知栏模型兼容
- 🆕 **修复qtai-sj无前缀转发模型错误** — 有前缀脑子回复后不再覆盖 activeNodeName，确保无前缀转发走用户选中的真实模型（如deepseek-v4-flash）
- 🆕 **统一ViewModel转发逻辑** — GatewayViewModel中qtai-sj路径均改用 activeNodeName 替代虚拟模型ID，避免查库失败
- 🆕 **lastRealModel备份机制** — 用户切换qtai-sj时自动保存上一个真实模型，防止丢失

## 🔄 v3.7.6（正式发布）
### 🚀 qtai-sj无前缀转发修复 + 超时优化
- 🆕 **修复qtai-sj无前缀不回复** — 选qtai-sj不喊綦小桐时，用 `activeNodeName`（已切换的真实模型）转发上游
- 🆕 **升级MAX_RETRIES 3→5** — 故障转移重试加多，减少500错误
- 🆕 **脑子超时30s/60s** — 大模型推理慢时不会卡死了
- 🔧 **脑子回复后强制return** — 避免脑子回复后继续走透传导致异常
- 🔧 `forced == "qtai-sj"` 时不再查数据库，改用 `activeNodeName` 正确找真实模型

## 🔄 v3.7.5（已废弃）
### 🚀 修复总输入始终为0 + 补全统计覆盖
- 🆕 **硬指令路径补总输入** — `綦小桐 切换xxx` 等工具指令现在正确累加 `trafficUploadBytes`
- 🆕 **硬指令路径补模型名** — 工具指令执行后通知栏显示 `activeNodeName`
- 🔧 **全路径统计审计** — 确认5条路径全部覆盖：硬指令/脑子/首选模型/故障转移/非chat

## 🔄 v3.7.3（正式发布）
### 🔥 qtai-sj统计修复 + 通知栏实时更新
- 🆕 **通知栏同步模型切换** — 【指令:xxx】切换模型时，`activeNodeName` 立即更新，通知栏实时显示当前模型
- 🆕 **qtai-sj Token统计补全** — 脑子模型返回的 `usage` 正确累加 `tokenPromptInput`/`tokenCompletionOutput`
- 🆕 **qtai-sj记忆绑定** — 脑子回复内容同步写入 `BrainMemoryManager`（用户/AI 对话记忆）
- 🔧 修复 v3.6.8 引入的问题：qtai-sj 路径跳过了数据统计和记忆写入

## 🔄 v3.7.2（正式发布）
### 🛡️ 代码审查全面修复 + API密钥验证 + 访问日志
- 🆕 **API密钥验证** — `validateApiKey()` 拦截器，可在管理页开关，白名单列表管理
- 🆕 **访问日志系统** — 环形缓冲区 1000 条，记录 IP/方法/路径/模型/状态/耗时
- 🆕 **`/v1/logs` 端点** — 查看实时访问日志
- 🆕 **健康检查全面完善** — 返回 version/uptime/模型数/日志数/API密钥状态 等详情
- 🎯 **模型切换优先级修复** — 精确匹配→数字→关键词模糊搜索排行榜，不再乱跳到简写
- 📝 **多模态文本提取统一** — `extractTextContent()` 统一处理字符串/数组格式
- 💰 **Token统计透传** — 脑子模型返回真实usage，工具指令估算prompt_tokens
- 🔑 **会话key精准** — 用完整key的hashCode，不再前20位冲突
- 🖼️ **qtai-sj图片请求自动路由** — 检测图片请求自动选🔁视觉模型
- 🚫 **无可用模型时返回503+JSON** — 不再静默500
- 🔧 全局异常处理统一返回JSON错误体

## 🔄 v3.7.1（正式发布）
### 🐛 豆包调试报告6项修复
- 🆕 **SSE格式标准化** — 所有chunk统一加 `data:` 前缀，首个chunk不遗漏
- 🆕 **500错误统一JSON响应体** — `openAIError()` 返回标准JSON格式 `{"error":{"message":"..."}}`
- 🆕 **无关键词500错误兜底** — 故障转移 + 503明确错误体
- 🆕 **多模态消息格式修复** — content为数组时正确解析提取文本
- 🆕 **Token统计正确透传** — 流式/非流式均透传 `completion_tokens`
- 🆕 **健康检查增强** — 返回 version/port/models_count/running 等详细状态
- 🔧 模型切换持久化修复

## 🔄 v3.7.0（正式发布）
### 🧑 人格名称全程脑子处理，自由对话智能体
- 🆕 **自由思考+回复** — 喊"綦小桐"后脑子自己回复，不透传
- 🆕 **自然语言理解** — 支持"你有几块糖"等日常对话
- 🆕 **指令与聊天一体化** — 脑子可同时处理指令（`【指令:xxx】`）和自由聊天
- 🆕 **智能判断** — 脑子自己决定是否需要转发上游模型
- 🔧 temperature 0.01→0.7，max_tokens 200→500
- 🔧 不再输出JSON指令格式，改为自然语言+指令标记

## 🔄 v3.6.9（正式发布）
### 🏗️ 进程树重构——拆分GatewayScheduler调度层
- 🆕 **GatewayScheduler 单例** — 从 GatewayService 拆出独立调度层
- 🆕 **超预索引** — 启动时全量预排序缓存
- 🆕 **staging原子操作** — snapshot→commit→rollback
- 🆕 **降级日志** — 9种原因不静默
- 🆕 **并行热槽组** — async+awaitAll 真并发热探测
- 🔧 路由层不再直接管理模型调度

## 🔄 v3.6.8（正式发布）
### ⚡ qtai-sj透传转发修复
- 🆕 **脑子说chat后直接透传** — 不走复杂路由（模型排序/记忆/人格）
- 🔧 消除卡顿和断连问题
- 🔧 qtai-sj流式输出优化

## 🔄 v3.6.7（正式发布）
### ⚡ SSE流式卡顿修复 + 指定模型切换
- 🆕 **SSE流式一次性输出** — 取消逐字 delay(20)，消除卡顿
- 🆕 **指定模型切换修复** — regex升级支持 `/` 字符
- 🔧 故障转移 MAX_RETRIES 5→3
- 🔧 脑子 max_tokens 200→300
- 🔧 删除文字扫描误设强制模型逻辑

## 🔄 v3.6.6（正式发布）
### 🧠 全智能思考 + 模型能力标记
- 🆕 **模型能力标记注入脑子** — 排行榜显示 🎨图片/🛠️工具/👁️视觉/💬聊天 标记
- 🆕 **根据能力标记推荐** — 脑子只推荐有对应能力的模型（如图片需求→只选🎨）
- 🆕 **文字兜底提取** — 脑子输出自然语言时，自动扫描文字中模型ID/名称并切换
- 🆕 **非chat路径修复** — `/v1/images/generations` 根据model字段找正确服务商
- 🔧 brainPrompt升级为JSON模式+示例，max_tokens提升到200，temperature=0.01
- 🔧 脑子返回文字时自动匹配排行榜模型并强制切换

---

## 🔄 v3.6.5（正式发布）
### 🧠 全智能思考系统
- 🆕 **脑子带排行榜分析** — brainPrompt注入完整排行榜数据
- 🆕 **智能推荐模型** — 脑子根据模型名称分析能力（图片/视频/代码等），自动推荐并切换
- 🆕 **forceModel脑子解析** — 脑子返回的JSON中提取params.modelId自动切换
- 🆕 **全自动判断** — 用户说"找能生成图片的模型"，脑子自己分析排行榜并切换

### 🔧 优化
- 🔧 brainPrompt 升级为完整智能助手，不再仅限于指令判断
- 🔧 排行榜数据实时注入脑子模型，让脑子知道全部模型信息

---

## 🔄 v3.6.4
### 🧠 大脑记忆注入网关
- 🆕 **buildPersonaPrompt输出记忆完整内容** — 不只是标题
- 🆕 **getSubconscious补充短期记忆** — 不足5条时自动补最近记忆
- 🆕 **网关转发注入记忆+人格** — 上游模型知道你在聊什么

---

## 🔄 v3.6.3
### 🔢 排行编号+当前模型显示
- 🆕 **排行带编号** — 綦小桐 排行 返回编号列表
- 🆕 **编号切换** — 回复数字直接切到对应模型
- 🆕 **查询当前模型** — 綦小桐 当前模型
- 🆕 **首页显示AI模型** — 强制选择上方显示当前模型
- 🔔 **通知栏联动** — 切换模型时日志显示

---

## 🔄 v3.6.2
### ✅ 指令系统调试通过
- 🆕 全面测试通过：硬指令、前缀、脑子模型全部正常

---

## 🔄 v3.6.1
### 🧑 人格名称动态绑定
- 🆕 管理页人格名称自动同步到前缀系统
- 🆕 Toast提示替代Snackbar
- 🆕 切换指定模型ID修复

---

## 🔄 v3.6.0
### 🔥 前缀指令系统
- 🆕 必须加綦小桐/qtai-sj/XiaoTong/自定义人格名才响应
- 🆕 不加前缀直接走正常路由
### 🧠 新增：记忆大脑系统
- 🆕 **BrainMemoryManager** — 独立记忆系统，SharedPreferences存储，不升级数据库（v5不变）
- 🆕 **三级记忆架构** — 短期记忆 → 自动升级为长期记忆 → 潜意识影响行为
- 🆕 **情感感知** — 自动检测 happy/sad/angry/surprised/neutral 情感标签
- 🆕 **重要性评分** — 自动计算0-10分，关键词+长度加权
- 🆕 **保存模式** — 频繁/正常/偶尔三种模式控制记忆密度
- 🆕 **记忆管理UI** — 管理页完整卡片：展开列表、搜索、编辑、删除、情感选择、重要性设置

### 🌐 语言切换修复
- 🔧 **AppCompatDelegate** — 改用 `setApplicationLocales()` 替代手动 `updateConfiguration`
- 🔧 **MainActivity** — 继承 `AppCompatActivity`，切换语言后系统自动 recreate
- 🔧 **appcompat:1.7.0** — 新增依赖

### 🔌 模型能力标记
- 🆕 **ModelCapabilityManager** — SharedPreferences 存储模型能力（supportsTools/supportsVision/supportsImageGen）
- 🆕 **probeModel()** — 测速结束后自动发探针检测模型能力
- 🆕 **自动切换** — 发送消息时能力不匹配自动切换到排行榜下一个支持该能力的模型

### 🔧 修复
- 🐛 **语言切换UI不刷新** — StateFlow响应式 + AppCompatDelegate 系统级切换
- 🐛 **备份EPERM权限** — Android 10+ 改用 MediaStore 方式写入
- 🐛 **Factory冲突** — `Icons.Default.Factory` 和 `GatewayViewModel.Factory` 重名修复

### 📦 其他
- 数据库保持 v5 未升级
- 全部新功能通过 SharedPreferences / 内存实现

---

## 🔄 v3.4.1
### ✨ 新增
- 🆕 **手动强制切换模型** — 排行榜上点击任意模型，立即强制切换到该模型
- 🆕 **📡 实时会话跑马灯** — 首页显示网关实时调用流，带时间戳，从右向左滚动
- 🆕 **歌词式会话流** — 请求一来显示「📤 发送」，回复了显示「📥 回复」，实时更新
- 🆕 **用户消息提取** — 跑马灯显示用户发送的原文（不是JSON），普通人也能看懂
- 🆕 **官方QQ群** — 用户交流群：`1007488535`

### 🔧 修复
- 🐛 **模型页测速空白回复** — 解析返回内容确认有回复，空白=失败
- 🐛 **首页自动测速同样问题** — 排行榜正确显示✅/❌
- 🐛 **跑马灯UI缺失** — 恢复滚动动画，每条错落有致

### 🔄 重构
- 🔄 **排行榜可点击** — 点排行项强制切换，再点取消
- 🔄 **实时会话全链路** — 请求→发送→回复，完整生命周期显示

---

## 🔄 v3.4.0
### ✨ 新增
- 🆕 **聊天室输入框上方思考小字** — AI回复时显示"💭 AI 正在思考..."
- 🆕 **消息操作菜单** — 点击消息气泡弹出操作按钮
- 📋 **消息长按复制** — 长按任意消息自动复制到剪贴板
- ✏️ **用户消息编辑重发** — 编辑后保存并自动重新发送
- 🔄 **用户消息重发** — 一键将内容填入输入框重新发送
- 📤 **系统分享** — 通过系统分享菜单分享消息内容
- 🔄 **AI消息重生成** — 删除最后一条AI消息并自动重新发送
- 💡 **消息时间戳显示** — 每条消息底部显示 HH:mm 时间

### 🔧 修复
- 🐛 **自动故障转移初始化联动** — 启动时自动检测故障转移状态并启动测速，不再需要关/开才生效
- 🐛 **聊天室qtai-sj请求失败** — 改为通过本地网关转发，网关自动选最优模型
- 🐛 **AI返回过快卡闪退** — 优化流式消息更新机制，分离编辑模式与消息列表
- 🐛 **空响应/空白内容触发故障转移** — 上游返回空body或空choices时自动切换模型

### 🔄 重构
- 🔄 **qtai-sj → 🔄 自动化切换** — 全项目名称统一替换
- 🔄 **ModelCard 开关绑定** — 自动化切换模型开关直接绑定独立状态
- 🔄 **故障转移与测速联动** — 开启故障转移自动启动测速，关闭自动停止
---
## 🔄 v3.3.6
### 🐛 Bug修复
- 🐛 `markModelSuccess`传真实延迟而非0（正确记录最快模型）
- 🔧 usage正则`[^}]+`→`[^{}]+`避免嵌套花括号截断
- ⚡ traffic统计`var`→`AtomicLong`（多线程并发不丢数）
- 💰 故障转移探活`max_tokens=3`→`1`（节省API配额）
- 🔳 通知栏点击不新开窗口（singleTask）
- 📊 流水线排行榜Activity重建不丢失（包级全局变量）
### ✨ 新增
- ⚡ **綦桐AI测速虚拟模型（qtai-sj）** — 模型页/聊天/网关三端同步显示
- 🏆 **模型页新增「⚡ 最佳模型同步」分组** — qtai-sj独立显示
- 🌐 **网关`/v1/models`返回qtai-sj** — 第三方APP也可选qtai-sj
- 🎯 **qtai-sj独立于autoFailover** — 不管开关状态，永远走排行榜最快模型
- 🤝 **用户模型永远优先**（失败后才按排行切换）
- 📊 **排行榜始终显示**（不管测速中还是停止）
- 💾 **代理测速走代理**（代理开启时自动走代理）

---
## 🔄 v3.3.5
### ⚡ 优化
- ⚡ 流式输出优化（边读边写，消除卡顿）
- 🧠 会话级模型记忆（同一会话优先用上次成功的模型）
- 🎯 请求模型优先（用户选的模型永远放第一位）
- 🔄 通知栏实时显示模型名（标题+正文）
- 🚀 通知栏更新频率 3s→1s

---

## 🔄 v3.3.1
### 🔧 修复
- ❌ 代理开关异常修复（开关交互、状态同步）
- ❌ 测速逻辑优化（先测海外，不通再测国内）
- ❌ 代理节点导入解析全面精简优化
- ❌ 非代理协议无效分支清理

---

## 🔄 v3.3.0
### 🆕 新增
- **🌐 多语言系统** — 支持15种语言（中文/英文/日文/韩文/法文/德文/西班牙文/俄文/葡文/越南文/泰文/阿拉伯文/印地文/印尼文）
- **🔄 语言自动跟随** — 自动检测手机系统语言，开启后无缝切换
- **👆 手动语言切换** — 管理页新增「🌐 语言设置」卡片，可自由选择15种语言
- **🧠 翻译管理器** — 全局翻译引擎 `TranslationManager` + 快捷函数 `tr(key)`
- **🔧 最优模型记忆 + 4xx/5xx故障转移** — 前序更新整合

### ⚡ 优化
- 底部导航、首页、故障转移、管理页等多处文本已接入多语言

---

## 🔄 v3.2.3
### 🆕 新增
- **🧠 最优模型记忆** — 响应最快的模型自动记住（5分钟缓存），下次请求直接走最优模型
- **🔄 4xx/5xx 触发故障转移** — 上游返回 400/500 等错误时自动切换模型
- **⚡ 流式错误也转移** — 流式请求 4xx/5xx 同样触发故障转移

### 🔧 修复
- ❌ 上游报 `400 temperature invalid` 但不切换模型 → ✅ 自动抛异常切下一个模型
- ❌ 故障转移每次从头试 → ✅ 记住最快的模型，直接优先使用
- ❌ 第三方 APP 参数越界导致重试循环 → ✅ 修正后转发 + 出错直接切

---

## 🔄 v3.2.2
### 🆕 新增
- **🛡️ 请求体参数自动修正** — 自动修正 `temperature`（截取到 [0,2)）、`top_p`、`presence_penalty`、`frequency_penalty`、`max_tokens` 等越界参数
- **🧹 API 兼容性增强** — temperature 设为 2.5 等越界值不再报 400，自动修正为安全值

### 🔧 修复
- ❌ 上游 API 报 `Temperature invalid, should be in [0.0, 2.0)` → ✅ 自动截取到 `1.999`
- ❌ 第三方 APP 发送越界参数导致重试循环 → ✅ 修正后再转发

---

## 🔄 v3.2.1
### 🆕 新增
- **🧠 智能故障转移完全体** — 开启自动切换后，后台并发测速所有启用模型（5秒超时），按响应速度排序
- **⚡ 模型健康缓存** — 测速结果缓存60秒，**最快的模型优先使用**，失败的自动标记不可用
- **🔄 逐级切换** — 一个模型失败→标记为不健康→立即尝试下一个最快模型→直到有模型成功返回
- **🔍 后台自动刷新** — 每个请求触发时检查缓存是否过期，过期自动刷新测速数据

### ⚡ 优化
- **🧵 彻底修复阻塞 IO** — 所有 OkHttp 阻塞调用用 `withContext(Dispatchers.IO)` 切到 IO 线程池
- **🚫 去除 runBlocking** — Ktor 路由 handler 直接调用 suspend 函数，不阻塞 CIO 事件循环
- **📦 流式全量读取** — pipeStreamResponse 一次性读完上游响应再写回，避免 IO 线程阻塞写操作

### 🔧 修复
- ❌ OkHttp execute() 阻塞 Ktor CIO 事件循环 → ✅ withContext(Dispatchers.IO) 异步执行
- ❌ pipeStreamResponse 内 input.read() 阻塞写响应 → ✅ 全量读取后一次写入
- ❌ response.body?.bytes() 在 CIO 线程阻塞 → ✅ 全部在 IO 线程执行

---

## 🔄 v3.2.0
### 🆕 新增
- **🏠 根路径响应** — `GET /` 返回 JSON 状态信息，替代 404 空白，APP 不再报错
- **🔄 故障转移全面修复** — HTTP 请求失败时正确抛出异常，触发自动切换模型
- **⚡ 流式请求前置** — HTTP 请求移到 `respondBytesWriter` 外执行，失败才能触发故障转移
- **🔍 开启时自动检测** — 自动故障转移开启后，按服务商+模型启用状态智能选择候选模型

### 🔧 修复
- ❌ `http://localhost:8889/` 返回空白 404 → ✅ 返回 `{"service":"qitong-ai-gateway","version":"3.2.0","status":"running"}`
- ❌ pipeNormalResponse 异常被吞掉（catch 写错误响应不 re-throw）→ ✅ 抛出异常让 proxyRequest 捕获重试
- ❌ pipeStreamResponse HTTP 请求在 respondBytesWriter 内部执行（失败无法触发故障转移）→ ✅ 移到外部执行
- ❌ 故障转移永远不会触发 → ✅ 现在请求失败自动切换下一个模型，最多试5次

---


## 🔄 v3.0.1
### 🔧 修复
- ❌ 通知栏清零导致网关逻辑中断 — 流量满9秒无变化就归零，大模型思考中被打断
- ✅ **动态累计模式** — 像手机流量一样只增不减，永不归零！加 🟢传输中/⚪空闲 指示灯

---

## 🔄 v3.0.0
### 🆕 新增
- **内置网关抓包工具** — 管理页「网关抓包」卡片，实时记录请求/响应日志（含输入输出流量）
- **备份全面覆盖** — 新增 `代理列表` + `网关端口` 的备份与恢复

### 🔧 修复
- ❌ 网关长流式断开（`callTimeout(600s)` 限制）→ ✅ `callTimeout(0)` + `readTimeout(0)` 无上限
- ❌ OkHttp 无连接池 → ✅ `ConnectionPool(5, 30s)` 连接复用加速
- ❌ 抓包清空按钮 → ✅ 真清空（`clearDebugLogs()` 调用 clear）
- ❌ 抓包按钮颜色不变 → ✅ `debugMode` 改为 StateFlow，Compose 响应式变化

---

## 🔄 v2.6.1
### ⚡ 性能优化
- **网关流式转发提速** — 取消每块 `decodeToString`，改用 `ByteArrayOutputStream` 累积原始字节，流结束后只解码一次
- **去冗余 `runBlocking`** — 流式 usage 解析直接调用 suspend DAO，不再额外包一层

### 🗂️ 备份页面重构
- **合并为一个卡片** — 删除重复的「文件备份」「导入备份」独立卡片，整合到「备份 & 恢复」中
- **定时自动备份** — 新增 ⏰ 开关 + 时间设置（小时/分钟），可设置每天指定时间自动备份到 Downloads
- **布局精简** — 一键恢复、立即备份、定时开关都在一个卡片内完成

---

## 🔄 v2.6.0
### 🆕 新增
- **代理服务商单选互斥** — 开启一个代理时，其他代理自动禁用
- **按模型粒度控制代理** — 模型卡片新增 🔄/🔗 按钮，可单独设置每个模型是否走代理
- **UpstreamClient 直连客户端** — 新增 `getDirectClient()` / `getClientForModel(useProxy)`，不走代理的模型请求直连

### 🔧 修复
- ❌ 代理开关未互斥（开启一个后另一个还是开启状态）
- ✅ 开启一个代理时，自动关闭其他所有代理并持久化

### 🗄️ 数据库
- AiModel 新增 `use_proxy` 字段（默认 true）
- 数据库迁移 v4→v5：添加 `use_proxy` 列

---

## 🔄 v2.5.1
### 🆕 新增
- **模型别名输出修复** — 网关`/v1/models`保持`id`为原始`modelId`不变（**防止第三方客户端调API失败**），`display_name`展示别名，`custom_alias`字段单独返回

### 🔧 修复
- ❌ v2.5.0 误将`id`字段改为别名 → 第三方客户端用`id`调接口找不到模型
- ✅ 恢复`id`保留原始modelId，别名仅影响`display_name`和`custom_alias`展示

---

## 🔄 v2.5.0
### 🆕 新增
- **智能测速** — 同时并发测试百度和谷歌：谷歌通=🌍海外，仅百度通=🇨🇳国内，都不通=❌
- **HTTP/HTTPS代理修复** — 改用`java.net.Proxy`完整限定名解决与OkHttp冲突；HTTPS代理添加自签名SSL信任
- **自动备份 & 一键恢复** — 管理页新增卡片：点击「立即备份」导出到Downloads + 点击「一键恢复」扫描备份文件列表点选恢复
- **服务商禁用联动模型** — `getEnabledModels`改为`INNER JOIN providers p`，禁用服务商后模型自动隐藏

### 🔧 修复
- 测速只猜内外不准确（改为国内外同时测）
- 代理列表不能正确应用HTTP/HTTPS代理
- 服务商禁用但模型仍在列表显示

---

## 🔄 v2.4.1
### 🔧 修复
- 内容中疯狂出现 "null" 文字 — 流式响应`jsonPrimitive?.content`对JSON `null`值已过滤
- 聊天UI适配 — MessageBubble双层防御
- 更新日志保存到工作区 CHANGELOG.md

---

## 🔄 v2.4.0
### 🆕 新增
- 网关Token用量记录（非流式+流式自动解析 usage）
- API连接动态提示（输入地址实时显示最终URL）
- 模型别名输出（网关 `/v1/models` 返回 display_name + custom_alias）
### 🔧 修复
- 统计页面没有数据（网关代理未记录TokenUsage）
- 编辑服务商重复supportingText

---

## 🔄 v2.3.0
### 🆕 新增
- 代理订阅导入（一键订阅、Base64解码、批量去重）
- 剪贴板检测导入
- 手动添加VMESS/SS/VLESS/Trojan/Hysteria2
- 统计页网关流量
### 🔧 修复
- 订阅弹窗点击无响应
- 剪贴板导入闪退（String.startsWith冲突）
- 代理列表不可滑动
- 通知栏闲置不归零

---

## 🔄 v2.2.0
### 🆕 新增
- 代理列表可滑动
- 通知栏自动归零
- 代理卡片格式优化

---

## 🔄 v2.1.0
### 🆕 新增
- 手动添加代理支持扩展协议
### 🔧 修复
- 版本号真实更新
- 订阅/剪贴板崩溃

---

## 🔄 v2.0.0
### 🆕 新增
- 通用代理模式（支持/v1/*所有路径）
- 流式输出开关
- 一键订阅导入
- 剪贴板检测导入
- ProxyLinkParser解析器（vmess/ss/vless/trojan/hysteria2/socks5）
- 局域网IP自动获取+复制
### 🔧 修复
- 网关数据憋死（32KB缓冲区+管道直通）
- 所有Media Type支持
- 超时+body限制解除

---

## 🔄 v1.9.0（初始版本）
- 底部导航栏（首页/服务商/模型/聊天/统计/管理/关于）
- 服务商CRUD
- 模型同步+别名编辑
- 内置聊天
- 网关代理（Ktor Server）
- 代理管理（HTTP/HTTPS/SOCKS5）
- Token用量统计（仅内置聊天）
- 数据备份/恢复/重置
